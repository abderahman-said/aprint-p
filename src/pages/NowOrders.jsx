...
const NowOrders = ({ lang }) => {
  ...
  const fetcher = () => {
    return axios.get(`${process.env.REACT_APP_BACKEND_API}/orders?status=1`, {
      headers: {
          ...
          <Container>
            ...
            {serviceLoading || isFetching ? (
              ...
              <div className={`${styles1.servicesContainer}`}>
                {/* <Row>
                  {servicesList.map((item, index, arr) => (
                    <Col key={index} lg="4" xl="4" className="mt-5">
                      <div
                        to={`status?n1=${item.Order_Status.id}&n2=${item.Order_id}`}
