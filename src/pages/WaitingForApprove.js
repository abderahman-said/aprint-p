...
const WaitingForApprove = ({ lang, isUser }) => {
  ...
  const onServicesSuccess = (data) => {
    ...
    servData.subServiceName = data.data.data.subServiceName;
    servData.subServiceMainImg = data.data.data.subServiceMainImg;
  ...
  const servicesFetcher = () => {
    return axios.post(
      "https://dashboard.mobtkra-press.com/api/one/sub/service",
      {
        subServiceId: 2,
      },
      {
  ...
  const onSubmit = (values) => {
    if (!isUser) {
      ...
      formData.append("user_file", values.file);
  ...
  const fetcher1 = () => {
    return axios.post(
      "https://dashboard.mobtkra-press.com/api/service/request",
      formData,
      {
        headers: {
          lang: lang,
  ...
  return (
    <div className={`${styles1.currentSection} py-5`}>
      <Helmet title={serviceData.subServiceName}>
        <ModalMe
          show={modalShow}
          lang={lang}
                ...
                <Col lg="6" xl="6">
                  <div>
                    <Row className=" justify-content-lg-between">
                      <Col md={6}>
                        <h2 className={styles1.Approved}> Not Approved</h2>
                        <p className={` mb-2 ${styles1.wait}`}>
                          {" "}
                          Please Resend Details Again
                        </p>
                      </Col>
                      <Col md={6} className="  text-end">
                        <h2 className={styles1.OrderId}> Order ID :</h2>
                        <p className={` mb-2 ${styles1.wait}`}> ID124651</p>
                      </Col>
                    </Row>
                    <div>
                      <Formik
